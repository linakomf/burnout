...
router.post('/collections', authMiddleware, adminOnly, async (req, res) => {
  try {
    ...
    const ins = await pool.query(
      ...
      ['tmp', title.slice(0, 120), description, coverImg.path, sort_order]
    );
...
router.post('/', authMiddleware, adminOnly, async (req, res) => {
  try {
    const posterImg = requirePublicImagePath(req.body, 'poster_url', {
      required: true,
      label: 'постер',
    });
    if (posterImg.error) return res.status(400).json({ message: posterImg.error });
    ...
    const gallery_urls = parseGalleryUrlsField(req.body.gallery_urls);
...
router.patch('/:filmKey', authMiddleware, adminOnly, async (req, res) => {
  try {
    const filmId = parseFilmPublicId(req.params.filmKey);
    if (!filmId) return res.status(404).json({ message: 'Фильм не найден' });
    ...
    const posterImg = requirePublicImagePath(req.body, 'poster_url', { label: 'постер' });
    if (posterImg.error) return res.status(400).json({ message: posterImg.error });
    if (posterImg.path) {
      safeUnlinkUploadPath(uploadsAbs, existing.poster_url);
      patch.poster_url = posterImg.path;
    }
    ...
    if (Object.prototype.hasOwnProperty.call(req.body, 'gallery_urls')) {
      const nextGallery = parseGalleryUrlsField(req.body.gallery_urls);
      ...
      const kept = parseKeepGalleryUrls(req.body.keep_gallery_urls, existingGallery);
      const removed = existingGallery.filter((u) => !kept.includes(u));
      for (const u of removed) safeUnlinkUploadPath(uploadsAbs, u);
      patch.gallery_urls = JSON.stringify(kept);
    }
