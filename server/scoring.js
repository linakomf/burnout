...
function scoreGad7(questions, answers) {
  ...
  let level;
  if (sum <= 4) level = 'Низкая тревожность';else
  if (sum <= 9) level = 'Средняя тревожность';else
  level = 'Высокая тревожность';
...
function scoreMbiStudent(questions, answers) {
  ...
  let level;
  if (percentage < 40) level = 'Нет признаков выгорания';else
  if (percentage < 68) level = 'Риск выгорания';else
...
function scoreDaily5(questions, answers) {
  ...
  const percentage = max ? Math.round(sum / max * 100) : 0;
  let level;
  if (percentage < 35) level = 'Состояние стабильное';else
  if (percentage < 65) level = 'Повышенная нагрузка';else
  level = 'Нужен отдых и поддержка';
...
function scoreLikertSum(questions, answers) {
  ...
  const percentage = max ? Math.round(sum / max * 100) : 0;
  let level;
  if (percentage <= 33) level = 'Низкий';else
  if (percentage <= 66) level = 'Средний';else
  level = 'Высокий';
